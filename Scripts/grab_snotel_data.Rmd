...
snotel11 = snotel1 %>% 
  select(Date, starts_with('Air.Tem'))

head(snotel11)

names(snotel11) =c("date","Tavg", "Tmax", "Tmin","Tsoil")
...
```{r}
plot.data2 = snotel111 %>% 
  select(date, Tmax_C, Tmin_C) %>% 
  filter(date >= as.Date("2019-07-01") & date<= as.Date("2019-09-01")) %>% 
  reshape::melt(., id.vars="date") 
 


ggplot(plot.data2) +
  geom_line(aes(x =date, y=value, color=variable), lwd=1.2)+
   xlab("Date") +
...
plot.data22 = snotel22 %>% 
  filter(date >= as.Date("2019-07-01") & date<= as.Date("2019-09-01")) 


ggplot(plot.data22) +
  geom_line(aes(x=date, y=precip_flux), lwd=1.2)+
  plot.theme3+
